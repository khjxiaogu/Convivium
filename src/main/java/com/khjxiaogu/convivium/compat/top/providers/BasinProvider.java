...
public class BasinProvider  implements IProbeInfoProvider {
	...
	public void addProbeInfo(ProbeMode mode, IProbeInfo info, Player player, Level level, BlockState state, IProbeHitData hitResult) {
		if(level.getBlockEntity(hitResult.getPos()) instanceof BasinBlockEntity entity) {
			...
			if(player.isShiftKeyDown()&&entity.recipeHandler.getLastRecipe()!=null) {
				level.getRecipeManager().byKey(entity.recipeHandler.getLastRecipe()).flatMap(t->t.value() instanceof BasinRecipe?Optional.of((BasinRecipe)t.value()):Optional.empty())
				.ifPresent(r->{
					IProbeInfo layout=info.horizontal(info.defaultLayoutStyle().borderColor(0x88ffffff).spacing(0));
	...
	public ResourceLocation getID() {
		return TOPRegister.idForBlock("basin");
	}
