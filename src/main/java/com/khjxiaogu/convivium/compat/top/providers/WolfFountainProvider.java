...
public class WolfFountainProvider  implements IProbeInfoProvider {
	...
	public void addProbeInfo(ProbeMode mode, IProbeInfo info, Player player, Level level, BlockState state, IProbeHitData hitResult) {
		if(level.getBlockEntity(hitResult.getPos()) instanceof WolfFountainBlockEntity entity) {
			if(entity.item!=null) {
				info.item(entity.item);
			}
			if(!entity.fluid.isEmpty()) {
				info.tankSimple(entity.fluid.getFluidAmount(), entity.fluid.getFluid(),info.defaultProgressStyle().bounds(16, 16).showText(false));
	...
	public ResourceLocation getID() {
		return TOPRegister.idForBlock("fountain");
	}
